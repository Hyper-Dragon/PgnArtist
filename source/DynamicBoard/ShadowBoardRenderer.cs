...
public sealed class ShadowBoardRenderer : BoardRendererBase, IBoardRenderer
{
    ...
    public static void InternalPieceResize(int newSize)
    {
        foreach (KeyValuePair<char, Bitmap> pieceEntry in Pieces)
        {
            Bitmap bmpOut = new(newSize, newSize);
        ...
        foreach (KeyValuePair<char, Bitmap> shadowsEntry in Shadows)
        {
            Bitmap bmpOut = new(newSize, newSize);
    ...
    public override Task<byte[]> GetPngImageDiffFromFenAsync(in string fenString = "", in string compFenString = "", in int imageSize = 1024, in bool isFromWhitesPerspective = true)
    {
        ...
        {
            _logger?.LogDebug("Rendering board error [{FenString}] [{ExMessage}]", fenString, ex.Message);
