...
public sealed class ChessDotComBoardRenderer : BoardRendererBase, IBoardRenderer
{
    ...
    public override Task<byte[]> GetPngImageDiffFromFenAsync(in string fenString = "", in string compFenString = "", in int imageSize = 1024, in bool isFromWhitesPerspective = true)
    {
        ...
        try
        {
            ...
            _logger?.LogDebug(message: "Downloading board image from [{BoardUrl}]", boardUrl);
        ...
        {
            _logger?.LogDebug(message: "Download fom Chess.com timedout [{FenString}]", fenString);
        ...
        {
            _logger?.LogDebug(message: "Download from Chess.com error [{FenString}] [{ExMessage}]", fenString, ex.Message);
