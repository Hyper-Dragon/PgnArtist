...
internal class DiagramRenderer
{
    ...
    public Bitmap RenderMoveImageData(RenderableGameCollection renderableGames, string diagramTitle, float titleSize)
    {
        ...
        RenderFooter(graphics, image.Size, transBrush, shadowOffset: BoardShadowOffset);
                                      ...
                                      out Graphics graphics)
    {
        image = new((gridXLength * BlockSizeX) + SpacerSizeX + (SpacerSizeX / 2),
                    (gridYLength * BlockSizeY) + (BlockSizeY/3),
    ...
    private void RenderFooter(Graphics graphics,
                              Size imageSize,
                              Brush transBrush,
                              int shadowOffset = 0)
    {
        graphics.FillRectangle(transBrush,
                               SpacerSizeX,
                               imageSize.Height-(BlockSizeY/4)+shadowOffset,
                               imageSize.Width-(SpacerSizeX*2)+shadowOffset,
                               BlockSizeY/7);

        graphics.FillRectangle(new SolidBrush(Color.FromArgb(255, 50, 50, 50)),
                               SpacerSizeX,
                               imageSize.Height - (BlockSizeY / 4),
                               imageSize.Width - (SpacerSizeX * 2),
                               BlockSizeY/7);


        Font footerFont = new(FontFamily.GenericSansSerif, 10, FontStyle.Italic);
        string footText = "Generated by PgnArtist version x.x.x [https://hyper-dragon.github.io/PgnArtist/]";

        SizeF stringSize = graphics.MeasureString(footText, footerFont);


        graphics.DrawString(footText,
                    footerFont,
                    Brushes.Black,
                    ((imageSize.Width - SpacerSizeX) - stringSize.Width)+1,
                    ((imageSize.Height - (BlockSizeY / 4)) + (stringSize.Height / 3)+1));

        graphics.DrawString(footText,
                            footerFont,
                            Brushes.Orange,
                            (imageSize.Width - SpacerSizeX)-stringSize.Width,
                            (imageSize.Height - (BlockSizeY / 4))+(stringSize.Height/3));
    }
